...
sequenceDiagram
    ...
    ユーザー->>ターミナル: クライアントプログラムを起動
    ターミナル-->>ユーザー: ユーザー名を入力してください
    ユーザー->>ターミナル: 例）Taro

    ターミナル-->>ユーザー: モードを選択（1:作成, 2:参加）
    ユーザー->>ターミナル: 1 または 2 を入力

    alt ルーム作成
        ターミナル-->>ユーザー: ルーム名を入力してください
        ユーザー->>ターミナル: 例）MyRoom
        ターミナル-->>ユーザー: ルーム「MyRoom」を作成しました
    else ルーム参加
        ターミナル-->>ユーザー: 利用可能なルーム一覧を表示
        ターミナル-->>ユーザー: 参加するルーム名を入力してください
        ユーザー->>ターミナル: 例）MyRoom
        ターミナル-->>ユーザー: ルーム「MyRoom」に参加しました
    end

    note over ターミナル: ルーム名とトークンを内部で保持

    ターミナル-->>ユーザー: チャット開始。メッセージを入力してください
    loop チャット
        ユーザー->>ターミナル: メッセージを入力（例：「こんにちは！」）
        ターミナル-->>ユーザー: 送信メッセージ表示 & 他ユーザーのメッセージ受信
    end

    note over ターミナル: ユーザーが終了するまでループ継続
```
