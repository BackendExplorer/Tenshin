...
|----------|----------------|
| 開発言語 | Python 3.13.2 |
| 並行処理 / マルチスレッド | threading.Thread |
| プロトコル設計 | ヘッダー＋ボディ構造を独自設計（バイト列によるサイズ管理） |
| エラーハンドリング | try-except-finally 構文で通信エラー・例外に対応 |
| ロギング / デバッグ | `logging` モジュールでログ出力を整理 |
| インフラ / 実行環境 | Docker（開発・実行環境の統一） |
| その他 | Git / GitHub（バージョン管理と公開） |
