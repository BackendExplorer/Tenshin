...
sequenceDiagram
    actor ユーザー
    participant クライアント as クライアント(Python)

    %% プログラム起動と初期入力
    ユーザー->>クライアント: ① プログラムを起動
    ユーザー->>クライアント: ② ユーザー名を入力
    ユーザー->>クライアント: ③ 「1: ルーム作成」または「2: ルーム参加」を選択

    %% 条件分岐：ルーム作成 or ルーム参加
    alt 「1: ルーム作成」を選択
        ユーザー->>クライアント: ④ ルーム名を入力
        クライアント-->>ユーザー: ⑤ トークン受け取り（作成完了）
    else 「2: ルーム参加」を選択
        クライアント-->>ユーザー: ④ ルーム一覧を表示
        ユーザー->>クライアント: ⑤ 参加したいルームを選択
        クライアント-->>ユーザー: ⑥ トークン受け取り（参加完了）
    end
    ...
    loop チャット継続中
        ユーザー->>クライアント: ⑧ メッセージを入力・送信
        クライアント-->>ユーザー: ⑨ 他のユーザーからのメッセージ受信
    end
```
